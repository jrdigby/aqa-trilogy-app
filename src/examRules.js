// Exam metadata helpers — difficulty, paper targets, authoring guidelines.
// Board-specific constants live in src/examBoards/; AQA is the Phase 0 default.

import { getExamBoardRules } from "./examBoards/index.js";
import { DEFAULT_EXAM_BOARD, normalizeExamBoard } from "./sciencePath.js";
...
export function getPaperTargets(totalMarks, tier, subject, examBoard = DEFAULT_EXAM_BOARD) {
  const rules = getExamBoardRules(examBoard);
  const ao = largestRemainder(totalMarks, rules.AO_PAPER_RATIOS || { ao1: 0.4, ao2: 0.4, ao3: 0.2 });
  const demand =
    tier === "HT"
      ? largestRemainder(totalMarks, rules.HT_DEMAND_PCT || HT_DEMAND_PCT)
      : largestRemainder(totalMarks, rules.FT_DEMAND_PCT || FT_DEMAND_PCT);
  const subj = (subject || "biology").toLowerCase();
  const mathsPct = (rules.MATHS_MIN_PCT || MATHS_MIN_PCT)[subj] ?? 0.1;
  const rpPct = rules.RP_MIN_PCT ?? RP_MIN_PCT;
  return {
    marks: totalMarks,
    ao1: ao.ao1,
    ao2: ao.ao2,
    ...
    rp: Math.ceil(totalMarks * rpPct),
    examBoard: normalizeExamBoard(examBoard)
  };
}
...
export function getAuthoringGuidelinesHtml(examBoard = DEFAULT_EXAM_BOARD) {
  const rules = getExamBoardRules(examBoard);
  if (typeof rules.authoringGuidelinesHtml === "function") {
    return rules.authoringGuidelinesHtml();
  }
  return getExamBoardRules(DEFAULT_EXAM_BOARD).authoringGuidelinesHtml();
}
