...
import {
  ...
  resolveCalculationWorkflowRoot,
  resolveEquationIdForSubstitution,
  resolveMarkSchemeEquationId,
  resolveSubstitutionContext,
  resolveSubstitutionMarkScheme,
...
/**
 * GCSE mark scheme:
 * - Substitute (equation given): substitution + answer; +1 each optional rearrangement, conversion, sig figs
 * - Recall (equation sheet): equation select (unmarked) + substitution + answer; same optional steps
 */
...
export function buildSubstitutionFeedbackText(subStep, equation, ctx = {}) {
  const { slotEdits, promptOverrides, rearrangementSubject, convStep } = ctx;
  ...
  const parts = slotIds.map((id) => {
    if (convStep?.slot_id === id) {
      const si = convStep.answer != null
        ? String(convStep.answer)
        : firstSlotAnswerValue(subStep?.si_slot_answers?.[id] ?? subStep?.slot_answers?.[id]);
      if (!si || isSymbolicSlotValue(id, si)) return "";
      const unit = convStep.to_unit || getSlotSiUnit(id) || "";
      return `${id}=${si}${unit}`;
    }
    if (slotEdits?.[id]) {
      const edit = slotEdits[id];
      const display = edit.isConversionSlot ? (edit.si ?? edit.display) : edit.display;
      if (isSymbolicSlotValue(id, String(display ?? ""))) return "";
      const unit = edit.isConversionSlot
        ? (edit.toUnit || getSlotSiUnit(id) || "")
        : (edit.unit || getSlotSiUnit(id) || "");
      return `${id}=${display}${unit}`;
    }
    if (promptOverrides?.[id]) {
      return `${id}=${promptOverrides[id]}`;
    }
    const markVals = subStep?.si_slot_answers?.[id] ?? subStep?.slot_answers?.[id];
    return formatSlotForSubstitutionFeedback(id, markVals);
  }).filter(Boolean);
...
/** Show rearrangement subject + mode panels when the rearrangement step is enabled. */
export function updateRearrangementAuthoringUi(prefix = "") {
  const p = (id) => document.getElementById(prefix + id);
...
export async function refreshRearrangementSubjectFromForm(prefix = "", supabaseClient = null) {
  const p = (id) => document.getElementById(prefix + id);
  ...
  const current = p("CalcRearrangementSubject")?.value?.trim()
...
export function resolveMarkingContext(config, resp, equationSheet, steps) {
  ...
  const selectedEqId = resp?.steps?.equation_select
    || (typeof subPayload === "object" && subPayload?.equation_id)
    || subStep?.equation_id
    || markSchemeEqId;
...
export function buildWrongEquationFeedback(markSchemeEquation, cleanUrl, steps, markPoints, stepResults = {}) {
  const items = [];
  const eqStep = steps.find((s) => s.type === "equation_select");
  const latex = markSchemeEquation?.latex || "";
  const plainEquation = latexToPlainOptionText(latex)
    || markSchemeEquation?.label
    || markSchemeEquation?.id
    || "";
  ...
  if (eqStep) {
    items.push({
      ...
      text: plainEquation
        ? `The correct equation is: ${plainEquation}`
  ...
  // Unit conversion is always marked in the main loop — independent of equation choice.
...
function resolveExpectedRearrangementAnswer(step, config, steps, resp, equationSheet, conversionEcf = null) {
  ...
  const isNumeric = step.mode === "numeric"
    || (isStructuredSubstitutionStep(subStep) && step.mode !== "symbolic");
  if (!isNumeric) return step.answer || "";

  const subPayload = resp?.steps?.substitution;
  const selectedEq = (typeof subPayload === "object" && subPayload?.equation_id)
    || resp?.steps?.equation_select
    || subStep.equation_id;
...
/** When conversion is wrong, derive ratio for AQA error-carried-forward through later steps. */
export function resolveConversionEcfState(steps, resp) {
  const convStep = steps.find((s) => s.type === "conversion");
...
function buildConversionEcfNotice(stepType, conversionEcf) {
  const valueText = conversionEcf?.studentVal != null
    ? `your converted value of ${conversionEcf.studentVal}`
    : "your conversion";
  return `Error Carried Forward (ECF): ${stepType} marked correct using ${valueText}.`;
}

/** Re-evaluate the final answer from the student's substituted SI values (handles e² etc.). */
export function resolveWorkflowDerivedAnswer(config, steps, resp, equationSheet, conversionEcf = null) {
  const subStep = steps.find((s) => s.type === "substitution");
  if (!subStep?.slot_answers || !equationSheet) return null;

  const markSchemeEqId = resolveMarkSchemeEquationId(config, subStep);
  const subPayload = resp?.steps?.substitution;
  const eqId = markSchemeEqId
    || (typeof subPayload === "object" && subPayload?.equation_id)
    || subStep.equation_id;
...
/** Dedicated sig figs input after the calculation answer. */
...
export function renderCalculationWorkflow(q, currentKey, presentation = "practice", equationSheet = null) {
  ...
  for (const step of steps) {
    ...
    if (step.type === "equation_select") {
      ...
      if (step.mode === "numeric" && subStep && equationSheet && !rearrLocked) {
        ...
        if (eqId) {
          ...
          const studentSlots = ctx.mode === "structured" && ctx.template
            ? collectStructuredSubstitution(ctx.template, resolveCalculationWorkflowRoot())
            : {};
      ...
      const sfNote = mergedSig && sigStep?.sig_figs
        ? ` <span style="font-weight:600;color:#64748b;">(to ${sigStep.sig_figs} s.f.)</span>`
        : "";
    ...
    if (step.type === "calculate" || step.type === "conversion") {
      ...
      if (typeof val === "object" && val?.mode === "structured") {
        ...
        if (!step.rearrangement_subject && step.equation_id && equationSheet) {
          ...
          const template = getSubstitutionTemplate(equation);
          const resultSlot = identifyResultSlot(template);
          if (resultSlot) omitSlotIds = [resultSlot];
        }
...
/** Baked authoring feedback can show stem units (e.g. m=10000g) — ignore when conversion step exists. */
...
/** Drop duplicate non-ECF lines for the same step (e.g. conversion reported twice). */
...
function conversionStepWasCorrect(convStep, resp) {
  ...
  const target = parseFloat(convStep.answer);
  const tol = parseFloat(convStep.tolerance ?? 0.001);
  return Number.isFinite(studentVal)
    && Number.isFinite(target)
    && Math.abs(studentVal - target) <= tol;
}
...
export function markCalculationResponse(q, resp, key, markPoints, cleanUrl, equationSheet = null) {
  ...
  const workflowAnswer = conversionEcf
    ? resolveWorkflowDerivedAnswer(config, steps, resp, equationSheet, conversionEcf)
    : null;
  ...
  for (const step of steps) {
    ...
    if (step.type === "equation_select") {
      ...
      if (subMatch.match) {
        ...
        if (
          convStep
          && conversionStepWasCorrect(convStep, resp)
          && studentUsedStemValueInSubstitution(convStep, studentVal)
        ) {
        ...
        missing.push({
          ...
          text: getStepFeedback(
            ...
            markPoints,
            "substitution",
            buildLiveSubstitutionFeedback(step, config, equationSheet),
            config
          ),
          url: cleanUrl
        });
      ...
      const convCorrect = studentVal != null
        && studentVal !== ""
        && Math.abs(studentVal - conversionTarget) <= conversionTol;
      if (convCorrect) {
        earned = marks;
        isCorrect = true;
      } else {
      ...
      let calcCorrect = markingCtx.equationCorrect
        && studentVal != null
      ...
      if (!calcCorrect && markingCtx.equationCorrect && conversionEcf) {
        ...
        const scaledTol = workflowAnswer != null
          ? ansTol
          : Math.max(ansTol, ansTol * Math.abs(conversionEcf.ratio));
      ...
      const sigEcfNote = conversionEcf && workflowAnswer != null
        ? " based on your substituted values"
        : "";
...
/**
 * Build calculation_config for batch numeric generation (no DOM).
 * @param {'substitute'|'recall'|'substitution_only'|'equation_recall'|'rearrangement'} baseVariant
 * @param {{ equationId: string, sheetId?: string|null, slotAnswers?: object, siSlotAnswers?: object, conversion?: object|null, sigFigs?: number|null, rearrangementSubject?: string|null, includeRearrangement?: boolean }} opts
 */
export function buildCalculationConfigForVariant(baseVariant, opts = {}) {
  const {
    equationId,
    sheetId = null,
    slotAnswers = {},
  ...
  if (conversion) {
    steps.push({
      ...
      tolerance: conversion.tolerance ?? 0.001,
  ...
  steps.push({
    ...
    rearrangement_subject: withRearrangement ? (rearrangementSubject || undefined) : undefined
  });
  ...
  if (withRearrangement) {
    steps.push({
      ...
      required: true,
      mode: "numeric",
      subject: rearrangementSubject || undefined
    });
  }
...
export function wireCalculationFormToggles(prefix = "", onChange, supabaseClient = null) {
  ...
  for (const [chkId, panelId] of pairs) {
    ...
    chkEl.addEventListener("change", () => { sync().catch(() => {}); });
    sync().catch(() => {});
  }
