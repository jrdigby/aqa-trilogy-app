...
- [x] Stop shipping full `answer_keys` / mark schemes to the browser for marking *(server `mark-response` preferred; prefetch gated to session/attempt access and limited lookahead)*.
- [x] Mark via Edge Functions (or RPCs) using the service role; return scores/feedback only.
- [x] Optionally serve practice questions via constrained RPCs (“next N for this session”) instead of open table reads of the whole bank. *(See migration `20260924190000_question_scrape_hardening.sql` + `src/questionDelivery.js`.)*
- [x] Revisit authenticated `SELECT` on `answer_keys` / `mark_points` once server-side marking is in place (likely developer-only or no client read).
