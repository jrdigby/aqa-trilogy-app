-- Fix srs_state RLS for student self-service + RPC for schedule seeding (bypasses RLS).
-- Run if dashboard shows: "new row violates row-level security policy for table srs_state"
...
-- Teachers read SRS for students in their classes (idempotent)
...
-- Insert seed rows as the authenticated user (security definer — not blocked by RLS)
