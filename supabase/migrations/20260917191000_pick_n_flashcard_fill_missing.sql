-- Also fill pick_n missing rows that never stored flashcard_text
-- (older attempts only had practice `text` with Acceptable answers).
      ...
      SELECT
        ...
        CASE
          WHEN NULLIF(trim(pool.full_label), '') IS NOT NULL
               AND NULLIF(trim(COALESCE(t.m->>'flashcard_text', '')), '') IS NULL
...
WHERE a.feedback_payload ? 'missing'
  ...
  AND EXISTS (
    ...
    WHERE NULLIF(trim(COALESCE(m->>'flashcard_text', '')), '') IS NULL
  );
